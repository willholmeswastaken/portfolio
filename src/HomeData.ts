...
export class HomeData {
  ...
  async getBlogPosts() {
    const blogPostsResponse = await fetch(
      "https://dev.to/api/articles?username=willholmes",
      { next: { revalidate: 3600 } }
    );
    const blogPostsParsed: Array<BlogPost> = await blogPostsResponse.json();
    const blogPosts: Array<BlogPostViewModel> = blogPostsParsed
      .slice(0, 3)
      .map((x) => ({
        id: x.id,
        title: x.title,
  ...
  async getPackages() {
    const packagesResponse = await fetch(
      "https://api.npms.io/v2/search?q=author:willholmeswastaken",
      { next: { revalidate: 3600 } }
    );
    const packagesParsed: PackagesResponse = await packagesResponse.json();
    const packages: Array<PackageViewModel> = packagesParsed.results.map(
      (x) => ({
